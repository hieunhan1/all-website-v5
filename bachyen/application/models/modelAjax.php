...
class modelAjax extends modelDB{
	...
	public function _viewDetail($table, $id, $datetime=NULL){
		if($datetime!=NULL) $datetime="AND `datetime`='{$datetime}'";
		$sql = "SELECT * FROM `{$table}` WHERE `id`='{$id}' {$datetime} LIMIT 1";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		return $result->fetch_assoc();
	}
	
	public function _updateStatus($table, $id, $user=NULL){
		if($user!=NULL) $user = ", `user`='{$user}'";
		$sql = "UPDATE `{$table}` SET `status`=1 {$user} WHERE `id`='{$id}' LIMIT 1";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		return true;
	}
