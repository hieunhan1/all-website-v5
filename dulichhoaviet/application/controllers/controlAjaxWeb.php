...
if(isset($_POST['rejectContact'])){
	...
	if($data!=false){
		$tour=''; $duration=''; $price='';
		$date=''; $address=''; $destination=''; $visitors=''; $children=''; $baby='';
		if($type==1){
			$type = 17;
		}else if($type==2){
			$type = 3;
			if(isset($_POST['header_id'])){
				...
				$tour = $dataHeader['name'];
				$duration = $dataHeader['duration'];
				$price = $dataHeader['price']; settype($price, 'int');
				$price = number_format($price, 0, ',', '.');
			}
		}else if($type==3){
			$type = 22;
			if(isset($_POST['date_khoihanh'])) $date = date('d-m-Y', strtotime($_POST['date_khoihanh']));
			if(isset($_POST['address'])) $address = $_POST['address'];
			if(isset($_POST['destination'])) $destination = $_POST['destination'];
			if(isset($_POST['visitors'])) $visitors = $_POST['visitors'];
			if(isset($_POST['children']) && $_POST['children']!='') $children = 'Trẻ em: '.$_POST['children'].'<em>(từ 2 tuổi đến 12 tuổi)</em>';
			if(isset($_POST['baby']) && $_POST['baby']!='') $baby = 'Trẻ nhỏ: '.$_POST['baby'].'<em>(dưới 2 tuổi)</em>';
		}else return false;
