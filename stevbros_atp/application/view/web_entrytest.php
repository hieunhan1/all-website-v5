...
<div id="wrapper">
	<div id="header">
    	<a href="http://www.stevbros.com" class="logo"><img src="http://www.stevbros.edu.vn/public/images/1450349056.png" alt="Stevbros" class="logo-1" /></a>
    	<a href="javascript:;"  class="logo" target="_blank"><img src="http://www.stevbros.edu.vn/public/images/1450349074.png" alt="PMI" class="logo-2" /></a>
    ...
    <div id="content" class="viewpost">
        ...
        <div id="entrytest">
			...
			//kiem tra da lam entry hay chua
			$entryTestData = $c->_model->_checkEntryTestData($key);
			...
			if(count($dataParent)!=0){
				foreach($dataParent as $rowParent){
					...
					foreach($dataEntry as $rowEntry){
						if(isset($entryTestData[$rowEntry['id']]['answers']))
							$rowEntry['user_answers'] = $entryTestData[$rowEntry['id']]['answers'];
...
$(document).ready(function(e) {
	...
	$("input[name=btnNext]").click(function(){ 
		...
		var total = parseInt(<?php echo $totalEntryTest;?>) - <?php if(count($entryTestData)==0) echo 1; else echo 2;?>;
	...
	function autoSendMail(){
		if(!$("#result .number").length) return false;
		var result = $("#result .number").html();
		
		var fields = new Object();
		fields['entrytestThongBao'] = 1;
		fields['result'] = result,
		fields['random_key'] = "<?php echo $rowTest['random_key'];?>";
		$.ajax({ 	
			url: 'ajax',
			type: 'POST',
			data: fields,
